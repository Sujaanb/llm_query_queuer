...
export function resolveProviderPermissionState(provider: ProviderMetadata, configuredEnabled: boolean, permissionGranted: boolean): ProviderPermissionState {
  ...
  return { id: provider.id, name: provider.name, phase: provider.phase, builtIn, status: planned ? 'planned' : provider.status as 'stable' | 'beta', enabled: planned ? false : builtIn || (configuredEnabled && permissionGranted), permissionGranted: planned ? false : builtIn || permissionGranted };
}
