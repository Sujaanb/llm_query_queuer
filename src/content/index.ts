...
import { assessQueueCapacity } from '../lib/queueLimits';
import { hasPromptContent } from '../lib/text';
import { readStorage, setPaused, updateQueue } from '../lib/storage';
import type { ProviderId, SelectorOverrides, Settings, TabStatus } from '../lib/types';
...
class ContentController {
  ...
  private routeUrl = location.href; private tabId: number | null = null; private providerId: ProviderId | null = null; private manualSubmitUntil = 0; private temporaryKey = `temp:unknown:instance:${crypto.randomUUID()}`; private routeTimer: number | null = null; private disposed = false; private selectorOverridesByProvider: Partial<Record<ProviderId, SelectorOverrides>> = {};
  ...
  private readonly message = (message: { type?: string; providerId?: string }, _sender: chrome.runtime.MessageSender, sendResponse: (response: unknown) => void) => {
    ...
    if (message.type === 'HIGHLIGHT_ELEMENTS') { const found = route && this.settings.debugMode ? route.provider.highlightDetectedElements() : []; sendResponse({ ok: found.length > 0, found }); return false; }
    if (message.type === 'GET_EFFECTIVE_SELECTORS') { sendResponse(route && this.settings.debugMode ? { ok: true, selectors: route.provider.getEffectiveSelectors() } : { ok: false }); return false; }
    if (message.type !== 'GET_STATUS') return false;
    const metadata = route ? runtimeProvider(route.provider.id) : null; sendResponse(route ? { supported: true, providerId: route.provider.id, providerName: route.provider.name, conversationId: route.provider.getConversationId(), conversationKey: route.key(), chatState: route.provider.getSnapshot().state, schedulerState: route.scheduler.state, isLeader: route.leader.current, experimental: Boolean(metadata?.experimental), accessDependent: Boolean(metadata?.accessDependent), diagnostics: this.settings.debugMode ? route.provider.getDiagnostics() : undefined } : { supported: false }); return false;
  };
  private readonly storageChanged = (changes: Record<string, chrome.storage.StorageChange>, area: string) => { if (area !== 'local') return; if (changes.settings?.newValue) this.settings = { ...DEFAULT_SETTINGS, ...changes.settings.newValue }; if (changes.selectorOverridesByProvider?.newValue) { this.selectorOverridesByProvider = changes.selectorOverridesByProvider.newValue as Partial<Record<ProviderId, SelectorOverrides>>; void this.reloadRoute(); } if (changes.providerEnablement?.newValue && this.providerId && this.providerId !== 'chatgpt' && !(changes.providerEnablement.newValue as Record<string, boolean>)[this.providerId]) this.dispose(); };
  ...
  async start(): Promise<void> {
    ...
    const storage = await readStorage(); this.settings = storage.settings; this.selectorOverridesByProvider = storage.selectorOverridesByProvider; if (!isProviderEnabled(runtime.id, storage.providerEnablement) || this.disposed) return;
  ...
  private async queueCurrent(): Promise<void> { const route = this.route; if (!route) return; const text = route.provider.readComposerText(); if (!hasPromptContent(text)) return; const storage = await readStorage(); const current = storage.queuesByProvider[route.provider.id]?.[route.key()] ?? []; const capacity = assessQueueCapacity(current.length, 1); if (!capacity.allowed) { this.toasts.show('Queue limit reached. Remove items before queueing more.', 'error'); return; } await updateQueue(route.provider.id, route.key(), (items) => [...items, makeItem(text, 'enter')]); await route.provider.clearComposer(); this.toasts.show(capacity.warning ? 'Message queued. Large queue: over 500 items.' : 'Message queued', capacity.warning ? 'warning' : 'success'); }
  ...
  private async initializeRoute(previousKey?: { providerId: ProviderId; key: string }): Promise<void> {
    const provider = createProviderAdapter(location.href, this.temporaryKey, () => this.settings, this.debug, this.providerId ? this.selectorOverridesByProvider[this.providerId] ?? {} : {}); if (!provider?.detect()) return; const conversationId = provider.getConversationId(); const nextKey = conversationId ? `conversation:${conversationId}` : provider.getTemporaryConversationKey(); const key = () => nextKey;
  ...
  private async reloadRoute(): Promise<void> { if (this.disposed || !this.route) return; const previous = { providerId: this.route.provider.id, key: this.route.key() }; this.route.dispose(); this.route = null; await this.initializeRoute(previous); }
