import type { ChatState, ProviderDiagnostics, ProviderId, SelectorOverrideKey } from '../../lib/types';
...
export interface ProviderSnapshot { state: ChatState; busy: boolean; ready: boolean; errorReason: string | null; uncertainReason: string | null; signals: readonly string[] }
...
export interface ProviderAdapter {
  ...
  isUncertain(): boolean; getUncertainReason(): string | null; getSnapshot(): ProviderSnapshot; getDiagnostics(): ProviderDiagnostics; isComposerEventTarget(target: EventTarget | null): boolean;
  classifyControlAction(target: EventTarget | null): 'send' | 'stop' | null; getEffectiveSelectors(): Partial<Record<SelectorOverrideKey, string[]>>;
  highlightDetectedElements(): string[]; observe(callback: () => void): () => void; dispose(): void;
}
