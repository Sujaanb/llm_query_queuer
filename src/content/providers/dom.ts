...
export function isElementVisible(element: Element, styleReader: StyleReader = (value) => getComputedStyle(value)): element is HTMLElement {
  ...
  if ('disabled' in html && Boolean((html as HTMLButtonElement).disabled)) return false;
...
export function findVisible(selectors: readonly string[], root: ParentNode = document, predicate: (element: Element) => boolean = isElementVisible): HTMLElement | null {
  for (const selector of selectors) for (const element of root.querySelectorAll(selector)) if (predicate(element)) return element as HTMLElement;
  return null;
}
export function recentVisible(selectors: readonly string[], limit = 4, root: ParentNode = document): HTMLElement[] {
  const result: HTMLElement[] = [];
  for (const selector of selectors) { const nodes = root.querySelectorAll(selector); let count = 0; for (let index = nodes.length - 1; index >= 0 && count < limit; index--) if (isElementVisible(nodes[index])) { result.push(nodes[index] as HTMLElement); count++; } }
  return result;
}
export function boundedText(element: Element, limit = 800): string {
  const text = element.textContent ?? ''; return text.length <= limit ? text : text.slice(0, limit);
}
...
export function composerKind(composer: HTMLElement | null): 'textarea' | 'contenteditable' | 'unknown' {
  ...
  if (composer?.isContentEditable || composer?.getAttribute('contenteditable') === 'true') return 'contenteditable';
  return 'unknown';
}
