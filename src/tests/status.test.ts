...
describe('localized status matching', () => {
  ...
  it.each([['Thinking', /thinking/iu], ['思考中', /思考中/iu], ['正在搜索', /正在搜索/iu], ['考え中', /考え中/iu], ['生成中', /生成中/iu]])('matches localized small status text %s', (value, pattern) => expect(statusPatternMatch(value, [pattern])).toBe(pattern));
